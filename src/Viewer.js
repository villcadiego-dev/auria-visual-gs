...
export class Viewer {
    ...
    addSplatScene(path, options = {}) {
        ...
        return loadFunc(path, format, options.splatAlphaRemovalThreshold, buildSection.bind(this),
            ...
            console.error('❌ Error inesperado en addSplatScene:', err);
        }
    }
    ...
    downloadSplatSceneToSplatBuffer(path, splatAlphaRemovalThreshold = 1, onProgress = undefined,
        ...
        // console.log('🌐 Descargando ksplat desde:', path);
        try {
    ...
    async setupSortWorker(splatMesh) {
        // console.log('🧠 Configurando sortWorker...');
        if (this.isDisposingOrDisposed()) return;
        ...
        // 🔧 Este "await" es fundamental para que luego podamos setear correctamente el .onmessage
        ...
        return new Promise((resolve) => {
            this.sortWorker.onmessage = (e) => {
                // console.log('📨 Mensaje recibido del sortWorker:', e.data);
                ...
                if (e.data.sortDone) {
                    ...
                    if (this.runAfterNextSort.length > 0) {
                        this.runAfterNextSort.forEach((func) => func());
                    ...
                    resolve(); // ✅ Muy importante para destrabar el .then() en addSplatScene
                }
            };
        });
    }
     ...
     */
    start() {
        // console.log('▶️ Iniciando render loop...');
